...
> **Source of truth:** The database is the source of truth for tool registrations inside Agent-Firewall. The tool registry YAML in the integration kit is a deployment artifact derived from DB state.
...
Agent-Firewall provides opinionated recommendations based on your tool
definitions. You don't have to figure out the right settings — the
system tells you:

### Automatic rules

| If your tool... | Agent-Firewall recommends... |
|-----------------|---------------------------|
...
After completing this step, Agent-Firewall generates:

```yaml
# tools.yaml — generated by Agent-Firewall
...
If your agent already uses OpenAI function calling format, Agent-Firewall
...
2. Agent-Firewall auto-fills name, description, args
