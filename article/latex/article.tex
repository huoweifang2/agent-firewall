...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.94\textwidth]{fig_trust_boundaries.pdf}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.96\textwidth]{fig_architecture.pdf}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.94\textwidth]{fig_proxy_pipeline.pdf}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.92\textwidth]{fig_intervention_flow.pdf}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.95\textwidth]{fig_tool_gate_state_machine.pdf}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.88\textwidth]{fig_delegation.pdf}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.92\textwidth]{fig_trace_evidence.pdf}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.92\textwidth]{fig_evidence_scope.pdf}
...
本文LaTeX正文中的结构图、流程图和示意图由\code{article/latex/generate\_autofigure\_figures.py}管理。该脚本会生成每张图的AutoFigure-Edit图稿说明，并在配置\code{AUTOFIGURE\_EDIT\_DIR}与\code{OPENAI\_API\_KEY}后调用外部AutoFigure-Edit CLI输出可编辑SVG，再转换为LaTeX引用的PDF；若本机未配置外部模型调用环境，脚本会生成带日志标记的本地fallback SVG/PDF，便于论文先完成编译和版面复核。数值图不使用AutoFigure生成，仍由\code{article/latex/generate\_figures.py}基于本地场景资产和论文固定口径绘制。推荐复现命令如下：

\begin{lstlisting}[language=bash]
cd /Users/isaachuo/Agent-Firewall
python3 article/latex/generate_autofigure_figures.py
...
python3 article/latex/generate_autofigure_figures.py --force-autofigure
\end{lstlisting}

其中\code{AUTOFIGURE\_REFERENCE\_IMAGE\_PATH}用于指定目标风格参考图。本文结构图的提示词按论文流程图风格组织：虚线外框、彩色横向泳道、左侧图标块、粗灰箭头和紧凑节点卡片；未配置真实AutoFigure-Edit时，本地fallback渲染器也按同一风格生成可编译PDF。
