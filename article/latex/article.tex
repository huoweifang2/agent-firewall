...
本文LaTeX正文中的结构图、流程图和示意图由\code{article/generate\_excalidraw\_flowcharts.mjs}管理。该脚本基于论文中已有的流程图节点和系统事实生成可编辑\code{.excalidraw}源文件，并导出Markdown、LaTeX和DOCX链路可复用的PNG、SVG和PDF图稿。数值图仍由\code{article/latex/generate\_figures.py}基于本地场景资产和论文固定口径绘制。推荐复现命令如下：

\begin{lstlisting}[language=bash]
cd /Users/isaachuo/Agent-Firewall/article
npm install
npm run figures:excalidraw
cd /Users/isaachuo/Agent-Firewall
python3 article/latex/generate_figures.py
\end{lstlisting}

Excalidraw源文件输出至\code{article/images/excalidraw\_sources/}，总览检查图输出为\code{article/images/excalidraw\_contact\_sheet.png}。结构图采用横向论文友好布局、语义配色和原生矢量图标，避免纵向长图插入论文后被压缩。
...
\filepath{article/generate_excalidraw_flowcharts.mjs} & Excalidraw结构图、流程图和示意图的可编辑源文件、PNG和PDF生成脚本。 \\
