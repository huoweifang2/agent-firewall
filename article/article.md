...
随着大语言模型应用从单轮问答逐步演进为能够调用工具、访问本机运行时、读取外部服务并委派子任务的智能体系统，传统 Web 应用中的访问控制、输入验证、审计追踪和凭据保护问题被重新引入自然语言接口。OpenClaw 与 MCP 类工具生态降低了智能体接入外部能力的成本，但也使提示词注入、工具滥用、混淆代理、间接注入、敏感信息泄露和多智能体委派链滥用等风险从模型输出层扩展到系统执行层。仅依赖模型自身拒答或提示词约束，难以证明一次工具调用是否经过权限校验、是否需要人工确认、工具结果是否安全进入上下文，以及异常行为是否留下可复核证据。

本文设计并实现了 Agent-Firewall，一个面向 OpenClaw 工具调用智能体的本机安全壳与 Web 运维控制台。系统位于消息入口适配器与 OpenClaw/MCP/Internal 工具之间，由 Proxy Service、Agent Runtime 和 Frontend 三部分组成。Proxy Service 提供 `/v1/scan` scan-only 输入扫描、策略配置、请求审计、Agent Control Plane 和 `/v1/interventions` 审批队列；Agent Runtime 负责受保护运行图、DeepSeek 模型调用、pre-tool gate、post-tool gate、OpenClaw/MCP provider 桥接和 Trace 证据链；Frontend 提供攻击演练、审批审计、Bot Agents、Skills & Hooks、Trace/Audit 和 Runtime Settings 等页面。系统源代码仓库地址为 `https://github.com/huoweifang2/agent-firewall`；如公开访问受限，复现实验以本地提交归档和随文代码文件为准。

本文围绕系统实现建立研究目标、案例库和实验口径。主实验使用 `apps/proxy-service/data/scenarios/playground.json` 与 `apps/proxy-service/data/scenarios/agent.json` 中的 358 个红队场景，其中包含 322 个预期阻断样本、16 个预期修改样本和 20 个安全放行样本；同时说明 `apps/proxy-service/src/red_team/packs/data` 下的 107 个 YAML 场景作为产品化 benchmark 资产。实验采用双 baseline：未保护直连 LLM 边界和 `fast` rules-only 策略，并与默认 `balanced` 策略对照。结果表明，未保护直连 LLM 在安全决策边界上仅能正确处理 20/358 个安全放行样本，`fast` 策略达到 269/358，`balanced` 策略达到 274/358；Agent 核心门控测试 125 项全部通过，工具链离线回放 11/11 通过。本机 OpenClaw 复测进一步表明，当前 active agent 能够通过 `/agent/chat` 调用 `openclaw_summarize`，并在 Trace-run 中记录 pre-tool、provider 执行、post-tool 截断和输入阻断证据。与此同时，`balanced` 离线口径仍有 84 个攻击或敏感样本被放行，失败集中在凭据格式多样化、多语言、编码混淆、认知操纵、RAG 污染和多轮渐进式攻击等场景。实验结果说明，Agent-Firewall 能够为工具调用智能体建立可解释的安全边界和审计闭环，但仍需在语义检测、多轮状态建模和生产化密钥治理方面继续完善。

**关键词：** 工具调用智能体；OpenClaw；MCP；提示词注入；访问控制；人工审批；Trace 审计

## Abstract

Large language model applications are moving from single-turn text generation toward agentic systems that can call tools, access local runtimes, consume external services, and delegate subtasks. This shift introduces access control, input validation, auditability, and credential protection issues into natural language interfaces. Tool ecosystems such as OpenClaw and MCP reduce integration cost, but they also expand the attack surface from model responses to executable system behavior. Prompt injection, tool abuse, confused deputy attacks, indirect prompt injection, sensitive data leakage, and unsafe delegation cannot be handled reliably by model refusal or prompt instructions alone.

This thesis designs and implements Agent-Firewall, a local safety shell and Web operator console for OpenClaw-based tool-calling agents. The system sits between message ingress adapters and OpenClaw/MCP/Internal tools. It consists of a Proxy Service, an Agent Runtime, and a Frontend. The Proxy Service provides the `/v1/scan` scan-only firewall endpoint, policy configuration, request audit logs, Agent Control Plane metadata, and `/v1/interventions` approval queues. The Agent Runtime executes the protected runtime graph, calls DeepSeek models, enforces pre-tool and post-tool gates, bridges OpenClaw/MCP providers, and records structured traces. The Frontend supports attack playgrounds, approval workflows, agent management, skill discovery, trace inspection, and runtime diagnostics.

The evaluation is grounded in local reproducible experiments. The main scenario inventory contains 358 JSON red-team cases from `playground.json` and `agent.json`, including 322 expected BLOCK cases, 16 expected MODIFY cases, and 20 expected ALLOW cases. Another 107 YAML scenarios are treated as benchmark assets rather than mixed into the main denominator. A dual-baseline design is used: an unprotected direct LLM boundary and a `fast` rules-only policy, compared against the default `balanced` policy. The unprotected boundary scores 20/358 on expected security-boundary decisions, the `fast` policy scores 269/358, and the `balanced` policy scores 274/358. Agent-layer gate tests pass 125/125 cases, and the offline tool-chain replay passes 11/11 cases. A local OpenClaw rerun further confirms that `/agent/chat` can execute the protected `openclaw_summarize` tool and persist Trace-run evidence for pre-tool checks, provider execution, post-tool truncation, and input blocking. The remaining failures are presented as capability boundaries rather than hidden: 84 attack or sensitive cases still pass under the current `balanced` offline configuration, mainly involving credential variants, multilingual attacks, obfuscation, cognitive manipulation, RAG poisoning, and gradual multi-turn attacks. The results show that Agent-Firewall provides an explainable and auditable safety boundary for tool-calling agents, while semantic detection, multi-turn risk accumulation, and production-grade secret governance remain important future work.
...
在传统 Web 应用中，权限校验、输入过滤、审计日志和人工审批通常由后端服务负责，用户界面只负责提交结构化请求。在工具调用智能体中，自然语言同时承载用户意图、上下文数据和潜在攻击指令，模型生成的工具计划又可能直接驱动后端能力。如果系统只依赖模型在提示词中“自觉拒绝危险请求”，就难以回答三个核心问题：第一，模型建议调用某个工具时，是否经过角色权限和参数 Schema 校验；第二，工具返回的外部内容是否可能包含间接提示词注入或敏感信息；第三，当工具调用被阻断或需要人工确认时，系统是否能够暂停、审批、恢复并留下完整证据。

OpenClaw 类本机智能体运行时进一步放大了这些问题。OpenClaw skills 和 hooks 可能触达本地配置、文件、账号凭据和外部服务。MCP 将模型与工具、数据源之间的连接标准化，降低了工具生态接入门槛，但也意味着攻击者可以围绕工具描述、参数 Schema、工具结果和多 Server 信任传播设计跨阶段攻击。智能体安全因此不能只讨论“模型是否生成危险文本”，还必须讨论系统是否能在模型之外证明“哪些动作被允许，哪些动作被拒绝，哪些结果可进入上下文，哪些证据可复核”。

本文研究的 Agent-Firewall 正是面向这一问题而设计的本机安全壳。它不把 Telegram 或某个聊天入口作为核心边界，而是把所有入口触发的请求统一送入受保护运行时。消息入口适配器负责接入外部消息，Proxy Service 负责输入扫描和审批状态，Agent Runtime 负责模型调用和工具门控，本地 Web 控制台负责攻击演练、审批和审计。通过这种分层设计，系统将模型规划和真实工具副作用隔离开来，使工具调用智能体在执行前、执行后和审计环节都具有可解释控制点。
...
本文的研究目标是设计并实现一个面向 OpenClaw 工具调用智能体的本机安全防火墙 Web 应用系统，使智能体在调用本机工具和外部 provider 时具备可控制、可审批、可追踪和可复现的安全边界。该目标可以分解为五个方面。
...
第四，建设可复现实验案例库。主实验使用 358 个 JSON 红队场景，覆盖普通 LLM 攻击、Agent 工具滥用、PII、密钥、混淆、多语言、RAG 污染和安全样本；同时保留 YAML 场景包作为产品化 benchmark 资产。

第五，以 baseline 和真实测试结果评价系统。本文采用未保护直连 LLM 边界和 `fast` rules-only 策略作为双 baseline，与 `balanced` 默认策略进行对照，并如实分析失败与 xfail 场景。
...
| --- | --- | --- |
| scan-only 输入扫描 | `apps/proxy-service/src/routers/scan.py`、`apps/proxy-service/src/pipeline/graph.py` | `/v1/scan`、`decision`、`risk_score`、`risk_flags` |
| 风险聚合与决策 | `apps/proxy-service/src/pipeline/nodes/decision.py` | `calculate_risk_score`、`decision_node` |
| Agent 运行图 | `apps/agent/src/agent/graph.py` | `AgentRuntimeGraph.ainvoke`、`tool_plan`、`final_response` |
| 工具前置门控 | `apps/agent/src/agent/nodes/pre_tool_gate.py` | `gate_decisions`、`pending_confirmation` |
| 工具后置门控 | `apps/agent/src/agent/nodes/post_tool_gate.py` | `sanitized_result`、`post_gate` |
| OpenClaw provider | `apps/agent/src/agent/tools/providers/openclaw.py`、`apps/agent/src/agent/openclaw_client.py` | `provider_type=openclaw`、`provider_ref` |
| intervention 审批 | `apps/proxy-service/src/routers/interventions.py`、`apps/agent/src/agent/interventions.py` | `pending`、`approved`、`completed` |
| Trace 证据链 | `apps/agent/src/agent/trace/accumulator.py`、`apps/proxy-service/src/control_plane/routers/trace_runs.py` | `pre_tool_decisions`、`tool_executions`、`post_tool_decisions` |
| direct baseline | `apps/agent/src/routers/chat.py`、`apps/proxy-service/src/routers/direct.py` | `/agent/openclaw/direct`、`/v1/chat/direct` |
...
`/v1/scan` 的入口实现直接调用 pre-LLM pipeline，并根据决策设置 HTTP 状态码。`BLOCK` 返回 403，`ALLOW` 或 `MODIFY` 返回 200。该接口只返回安全裁决，不转发到模型 provider。
...
async def scan(
    ...
    request_id = f"scan-{request.headers.get('x-correlation-id', uuid.uuid4().hex)[:24]}"
...
pre-LLM pipeline 的节点顺序也保持确定性。它先解析输入，再识别意图，随后应用规则与扫描器，最后由决策节点统一输出。
...
风险聚合的核心代码位于 `apps/proxy-service/src/pipeline/nodes/decision.py`。为便于论文表达，截取其关键逻辑如下。
...
决策节点在聚合风险后返回 `ALLOW`、`MODIFY` 或 `BLOCK`。如果 denylist 命中或风险超过策略阈值，则阻断；如果 PII 策略要求 mask，则返回 `MODIFY`；否则放行。该逻辑使系统能在不调用 LLM 的情况下完成可复现输入判断。
...
Agent Runtime 的运行图位于 `apps/agent/src/agent/graph.py`。它通过一个轻量 `AgentRuntimeGraph` 顺序执行节点，同时保留图式节点边界。核心循环如下。
...
class AgentRuntimeGraph:
    async def ainvoke(self, initial_state: dict[str, Any]) -> AgentState:
        ...
        while llm_turns < max_iterations:
            ...
            if state.get("pending_confirmation") or not state.get("tool_plan"):
                break
...
该运行图将模型规划和工具执行分离。LLM 节点只负责生成自然语言回复或工具计划；真实工具调用必须经过 pre-tool gate；工具结果必须经过 post-tool gate。循环次数受 `max_iterations` 限制，默认值为 3，防止工具调用无限循环。
...
当参数被安全截断或清洗时，pre-tool gate 返回 `MODIFY` 并使用修改后的参数继续执行；当工具敏感度需要人工确认时，返回 `REQUIRE_CONFIRMATION` 并设置 `pending_confirmation`，工具不会立即执行。
...
def evaluate_tool_output(
    ...
    working, pii_entities, pii_count = scan_pii(raw_result)
    working, secrets_count = scan_secrets(working)
    working, was_truncated = check_size(working)

    if was_truncated and pii_count + secrets_count > 0:
        decision = "REDACT"
    elif was_truncated:
        decision = "TRUNCATE"
    elif pii_count + secrets_count > 0:
        decision = "REDACT"
    else:
        decision = "PASS"
```

该模块的重要性在于，它把“工具执行成功”和“工具结果可被模型使用”区分开来。工具可能成功返回内容，但只要其中包含间接提示词注入，系统仍会阻断原始结果，避免模型在后续轮次中执行工具输出中的恶意指令。
...
OpenClaw provider 的执行并不是把用户原文直接交给本机 runtime，而是构造限定范围的 skill prompt，并派生独立 session id。关键实现如下。
...
真正调用 OpenClaw CLI 时，运行时使用 `openclaw agent --agent <agent_id> --session-id <session_id> --message <message> --json --timeout <seconds>`，并通过 `timeout_seconds=120` 控制最长等待时间。OpenClaw CLI 异常会转化为工具错误文本或 HTTP 502，而不会绕过 Agent-Firewall 的 Trace 与输出清洗链路。
...
async def agent_message(
    ...
    *,
    ...
    command = [
        ...
        "--message", message,
        "--json",
        "--timeout", str(timeout_seconds or self.timeout_seconds),
    ]
    stdout = await self.run(*command, timeout_seconds=timeout_seconds)
    return extract_json_payload(stdout.strip())
```

受保护入口与 direct 对照入口的差异也在代码层面固定。`/agent/chat` 使用 `get_agent_graph().ainvoke()` 进入完整运行图；`/agent/openclaw/direct` 则直接调用 OpenClaw，只用于 Compare 页面和 baseline 对照。
...
@router.post("/agent/openclaw/direct")
...
审批队列由 Proxy Service 持久化，Agent Runtime 和 Telegram Bridge 通过 HTTP helper 创建、查询和更新状态。核心 API 如下。
...
TraceAccumulator 在每个节点中增量记录证据。pre-tool gate 调用 `record_pre_tool_decision`，工具执行节点调用 `record_tool_execution`，post-tool gate 调用 `record_post_tool_decision`，最终响应节点调用 `finalize`。其中 pre-tool 与 post-tool 的记录格式如下。
...
Agent Runtime 默认参数位于 `apps/agent/src/config.py`：
...
Proxy/Compare 默认参数位于 `apps/proxy-service/src/config.py`：
...
系统仅支持 DeepSeek official API 模型。`apps/proxy-service/src/llm/providers.py` 中支持的模型为 `deepseek-chat` 和 `deepseek-reasoner`；非 DeepSeek 模型会被拒绝。
...
此外，系统还保留了 YAML 形式的产品化 benchmark 资产，位于 `apps/proxy-service/src/red_team/packs/data`。这些包包括：
...
本文主实验采用 JSON 场景库，因为其已与 `tests/test_scenario_coverage.py` 和 `tests/test_scenario_deterministic.py` 直接连接，能够形成稳定复现实验命令。YAML pack 用于说明系统已具备产品化 benchmark 模块，但不与 JSON 主实验混合计数。
...
两组策略的配置来自 `apps/proxy-service/src/db/seed.py`。`fast` 的 `nodes=[]`，仅保留基础 pipeline 与 `max_risk=0.9` 阈值；`balanced` 的 `nodes=["llm_guard", "nemo_guardrails", "output_filter", "memory_hygiene", "logging"]`，并把 `max_risk` 降为 `0.7`。因此，本文的 baseline 不是“换一个提示词”，而是在同一批场景、同一条 pre-LLM pipeline 上改变策略节点与阈值。
...
从表中可以看出，Agent-Firewall 的确定性安全边界显著优于未保护直连 LLM。未保护直连只能在安全 ALLOW 样本上“符合预期”，但无法提供系统级阻断或清洗。`fast` rules-only 已能覆盖大部分直接攻击，说明意图识别、规则聚合和风险阈值对毕业设计场景具有实际价值。`balanced` 在 `fast` 基础上进一步提升，但提升幅度有限，说明当前本地 scanner 配置并未完全解决语义伪装、多语言和凭据变体问题。
...
2. 实现了 `/v1/scan` scan-only 输入扫描，将意图、规则、扫描器、PII、密钥和自定义规则统一映射为 `ALLOW`、`MODIFY` 和 `BLOCK`。
3. 实现了工具执行前后双门控，使模型工具计划不能直接驱动真实工具副作用，工具结果也不能未经处理进入模型上下文。
...
第二，系统把工具调用前、工具调用后和模型调用前三个关键边界显式化，形成可审计状态机，而不是依赖提示词约束。人工审批被实现为后端 intervention 状态，不是简单的自然语言确认。

第三，系统把子 Agent 委派和 OpenClaw/MCP provider 调用都建模为受保护工具能力，使多 Agent 协作、本机 skill 执行和外部 provider 接入进入同一套策略体系。
...
```bash
cd /Users/isaachuo/Agent-Firewall
PYTHONPATH=apps/proxy-service PYTHONDONTWRITEBYTECODE=1 \
