...
- System shape: the main product path is `frontend -> proxy-service -> LLM provider`, with `agent-demo` showing in-agent guardrails and `reference-chat-target` acting as a benchmark target rather than a product service. See `apps/README.md:1-9`, `infra/docker-compose.yml:66-150`.
...
- Backend runtime: Python 3.12 with FastAPI, Pydantic Settings, LangGraph, LiteLLM, structlog, SQLAlchemy async, Alembic, asyncpg, and Redis. See `apps/proxy-service/pyproject.toml:1-35` and `apps/agent-demo/pyproject.toml:1-27`.
...
- `apps/agent-demo/`: the demo customer-support agent that scans input through the proxy and then applies its own tool guardrails. The important surfaces are `src/main.py`, `src/agent/graph.py`, `src/agent/nodes/`, `src/agent/rbac/`, and `src/agent/tools/`. Start with `apps/agent-demo/src/main.py:37-147`, `apps/agent-demo/src/agent/graph.py:1-151`, `apps/agent-demo/src/agent/state.py:1-104`, `apps/agent-demo/src/agent/nodes/pre_tool_gate.py:1-280`, and `apps/agent-demo/src/agent/nodes/post_tool_gate.py:1-257`.
