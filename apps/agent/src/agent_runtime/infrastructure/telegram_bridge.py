...
from agent_runtime.application.interventions import create_intervention, list_interventions, update_intervention
    ...
    for name, raw_account in _account_items(channel):
        ...
        accounts.append(
            TelegramAccount(
                ...
                drop_pending_on_start=bool(
                    account_bridge.get("dropPendingOnStart", bridge.get("dropPendingOnStart", True))
                ),
            )
        )
...
class TelegramBridge:
    ...
    async def _maybe_create_pause_intervention(
        ...
        pending_confirmation = (
            data.get("pending_confirmation") if isinstance(data.get("pending_confirmation"), dict) else None
        )
