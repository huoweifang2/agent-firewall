...
from src.agent.tools.orders import MOCK_ORDERS, get_order_status
...
def get_orders(order_id: str = "", customer_id: str = "") -> str:
    ...
    for order in MOCK_ORDERS.values():
        lines.append(
            f"- {order['order_id']} | {order['customer_name']} | {order['status']} | "
            f"{', '.join(order['items'])}"
        )
...
def search_products(query: str) -> str:
    ...
    for product in matches[:5]:
        lines.append(
            f"- {product['sku']} | {product['name']} | {product['category']} | "
...
def update_user(user_id: str, email: str = "", name: str = "", phone: str = "") -> str:
    ...
    return (
        f"User {normalized_user_id} updated: "
