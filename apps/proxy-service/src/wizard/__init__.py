...
New code should import from ``src.control_plane``. This package remains so
...
for _suffix in _ALIASES:
    _module = importlib.import_module(f"src.control_plane.{_suffix}")
    sys.modules[f"{__name__}.{_suffix}"] = _module
    if "." not in _suffix:
        globals()[_suffix] = _module

from src.control_plane import control_plane_router, seed_control_plane  # noqa: E402
from src.control_plane.seed import (  # noqa: E402
    REFERENCE_AGENT,
