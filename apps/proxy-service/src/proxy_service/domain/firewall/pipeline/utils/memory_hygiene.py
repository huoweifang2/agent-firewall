...
async def _redact_pii_from_messages(messages: list[dict]) -> list[dict]:
    ...
    from proxy_service.domain.firewall.pipeline.nodes.presidio import PII_ENTITIES, get_analyzer, get_anonymizer
    ...
    settings_mod = __import__("proxy_service.infrastructure.config", fromlist=["get_settings"])
...
def _redact_secrets_from_messages(messages: list[dict]) -> list[dict]:
    ...
    from proxy_service.domain.firewall.pipeline.nodes.output_filter import SECRET_PATTERNS
