...
\setCJKfamilyfont{zhsong}[AutoFakeBold=2.5,AutoFakeSlant=0.2]{Songti SC}
\setCJKfamilyfont{zhhei}[AutoFakeBold=2.5]{PingFang SC}
...
\newcommand{\decision}[1]{{\ttfamily\fontsize{7}{8}\selectfont #1}}
...
\begin{figure}[tbp]
  \centering
  \begin{tikzpicture}[x=\linewidth,y=7.4cm]
    \node[afbox, fill=afblue, text width=.34\linewidth] (web) at (0.50,0.90) {\textbf{Nuxt 4 + Vuetify Web 控制台}\\Playground / Agents / Traces / Analytics};

    \node[afbox, fill=afgreen, text width=.27\linewidth] (proxy) at (0.18,0.62) {\textbf{Proxy Firewall API}\\FastAPI :8000\\9 节点 LangGraph 文本防火墙};
    \node[afbox, fill=afyell, text width=.27\linewidth] (agent) at (0.82,0.62) {\textbf{Agent Runtime API}\\FastAPI :8002\\11 节点工具执行防护图};

    \node[afbox, fill=afgray, text width=.20\linewidth] (scan) at (0.18,0.38) {\textbf{本地扫描器}\\Rules / Intent\\LLM Guard / Presidio / NeMo};
    \node[afbox, fill=afgray, text width=.20\linewidth] (data) at (0.50,0.38) {\textbf{数据与状态}\\PostgreSQL / Redis\\策略 / 日志 / Trace};
    \node[afbox, fill=afgray, text width=.20\linewidth] (tools) at (0.82,0.38) {\textbf{工具提供者}\\Internal / MCP\\OpenClaw};

    \node[afbox, fill=white, text width=.18\linewidth] (llm) at (0.18,0.15) {\textbf{LLM Provider}\\LiteLLM 路由};
    \node[afbox, fill=white, text width=.18\linewidth] (langfuse) at (0.50,0.15) {\textbf{审计导出}\\Langfuse Span\\可选接入};
    \node[afbox, fill=white, text width=.18\linewidth] (subagent) at (0.82,0.15) {\textbf{Subagents}\\创建 / 绑定 / 委派};

    \node[afbox, fill=afteal, text width=.90\linewidth, inner ysep=5pt] (principle) at (0.50,0.02) {设计原则：将确定性安全决策与 LLM 业务推理解耦，Proxy 层负责文本风险，Agent 层负责能力边界与委派链。};

    \draw[afarrow] (web.south) -- ++(0,-0.03) -| (proxy.north);
    \draw[afarrow] (web.south) -- ++(0,-0.03) -| (agent.north);
    \draw[afarrow] (proxy.east) -- (agent.west);
    \draw[afarrow] (proxy.south) -- (scan.north);
    \draw[afarrow] (agent.south) -- (tools.north);
    \draw[afarrow] (proxy.south east) -- ++(0.02,-0.03) -| (data.north west);
    \draw[afarrow] (agent.south west) -- ++(-0.02,-0.03) -| (data.north east);
    \draw[afarrow] (scan.south) -- (llm.north);
    \draw[afarrow] (data.south) -- (langfuse.north);
    \draw[afarrow] (tools.south) -- (subagent.north);
  \end{tikzpicture}
...
\begin{figure}[tbp]
  \centering
  \begin{tikzpicture}[x=\linewidth,y=5.2cm]
    \node[afbox, fill=afblue, text width=.13\linewidth] (parse) at (0.10,0.73) {\textbf{parse}\\请求规范化};
    \node[afbox, fill=afblue2, text width=.13\linewidth] (intent) at (0.30,0.73) {\textbf{intent}\\意图分类};
    \node[afbox, fill=afgreen, text width=.13\linewidth] (rules) at (0.50,0.73) {\textbf{rules}\\规则命中};
    \node[afbox, fill=afyell, text width=.13\linewidth] (scanners) at (0.70,0.73) {\textbf{scanners}\\并行扫描};
    \node[afbox, fill=afred, text width=.13\linewidth] (decision) at (0.90,0.73) {\textbf{decision}\\风险聚合};

    \node[afbox, fill=aflav, text width=.15\linewidth] (transform) at (0.24,0.30) {\textbf{transform}\\脱敏与改写};
    \node[afbox, fill=afgray, text width=.15\linewidth] (llmcall) at (0.46,0.30) {\textbf{llm\_call}\\模型调用};
    \node[afbox, fill=aforange, text width=.15\linewidth] (output) at (0.68,0.30) {\textbf{output\_}\\\textbf{filter}\\输出过滤};
    \node[afbox, fill=afgray, text width=.15\linewidth] (logging) at (0.90,0.30) {\textbf{logging}\\审计日志};
    \node[afnote, fill=afred, text width=.15\linewidth] (block) at (0.90,0.08) {\decision{BLOCK}\\不调用模型};
    ...
    \draw[afarrow] (decision.south west) .. controls +(0,-0.08) and +(0,0.08) .. node[midway,left,font=\scriptsize]{\decision{MODIFY}} (transform.north);
    \draw[afarrow] (transform.east) -- (llmcall.west);
    \draw[afarrow] (llmcall.east) -- (output.west);
    \draw[afarrow] (output.east) -- (logging.west);
    \draw[afarrow] (decision.south) .. controls +(0,-0.10) and +(0,0.10) .. node[midway,right,font=\scriptsize]{\decision{ALLOW}} (llmcall.north);
    \draw[afdanger] (decision.south) -- (block.north);
  \end{tikzpicture}
...
\begin{figure}[tbp]
  \centering
  \begin{tikzpicture}[x=\linewidth,y=7.6cm]
    \node[afbox, fill=afgray, text width=.11\linewidth] (input) at (0.08,0.80) {\textbf{input}\\会话与限额};
    \node[afbox, fill=afgray, text width=.11\linewidth] (intent) at (0.24,0.80) {\textbf{intent}\\用户意图};
    \node[afbox, fill=afgray, text width=.11\linewidth] (policy) at (0.40,0.80) {\textbf{policy}\\\textbf{\_check}\\策略加载};
    \node[afbox, fill=afblue2, text width=.11\linewidth] (router) at (0.56,0.80) {\textbf{tool}\\\textbf{\_router}\\工具规划};
    \node[afbox, fill=afgreen, text width=.12\linewidth] (pregate) at (0.72,0.80) {\textbf{pre\_tool}\\\textbf{\_gate}\\RBAC / Schema / 限额};
    \node[afbox, fill=afyell, text width=.12\linewidth] (executor) at (0.88,0.80) {\textbf{tool\_}\\\textbf{executor}\\执行或委派};

    \node[afbox, fill=aforange, text width=.14\linewidth] (confirm) at (0.56,0.48) {\textbf{confirmation}\\敏感操作人工确认};
    \node[afbox, fill=afyell, text width=.12\linewidth] (postgate) at (0.72,0.48) {\textbf{post\_tool}\\\textbf{\_gate}\\PII / 密钥 / 注入清洗};
    \node[afbox, fill=afred, text width=.12\linewidth] (llmcall) at (0.88,0.48) {\textbf{llm\_call}\\Proxy 预扫描\\+ 模型调用};
    \node[afbox, fill=afgray, text width=.12\linewidth] (memory) at (0.72,0.18) {\textbf{memory}\\会话持久化};
    \node[afbox, fill=afgray, text width=.12\linewidth] (response) at (0.88,0.18) {\textbf{response}\\最终响应与 Trace};
    ...
    \draw[afarrow] (pregate.south west) .. controls +(-0.02,-0.09) and +(0,0.09) .. (confirm.north);
    \draw[afarrow] (executor.south west) .. controls +(-0.02,-0.09) and +(0.02,0.09) .. (postgate.north east);
    \draw[afarrow] (postgate.east) -- (llmcall.west);
    \draw[afarrow] (postgate.south) -- (memory.north);
    \draw[afarrow] (llmcall.south) -- (response.north);
    \draw[afarrow] (memory.east) -- (response.west);

    \node[afnote, fill=afgreen, text width=.26\linewidth] at (0.26,0.03) {第一道防线：执行前权限、参数与预算门控};
    \node[afnote, fill=afyell, text width=.24\linewidth] at (0.58,0.03) {第二道防线：工具结果清洗与阻断};
    \node[afnote, fill=afred, text width=.24\linewidth] at (0.86,0.03) {第三道防线：模型调用前的 Proxy 复扫};
  \end{tikzpicture}
...
\begin{figure}[tbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      ymax=1.0,
      ylabel={风险贡献上限或权重},
      symbolic x coords={Intent,Denylist,LLM Guard,NeMo,Presidio,Rules Boost},
      xtick=data,
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\textwidth}{p{2.5cm}p{2.8cm}p{2.1cm}Y}
    ...
    管理员密钥读取 & \code{getInternalSecrets} & \makecell[l]{\decision{REQUIRE\_}\\\decision{CONFIRMATION}} & admin具备权限，但工具敏感度为critical。 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
系统通过\code{apps/agent/src/agent/tools/hub.py}统一工具执行入口。执行顺序如下：如果工具名为\code{createSubAgent}，调用Proxy Service创建子Agent并刷新运行时缓存；如果工具名符合委派工具命名规则，例如委派工具\code{delegate_to_payments_analyst}，则根据运行时配置找到对应子Agent并调用OpenClaw Agent消息接口；否则读取工具spec，根据\code{provider_type}分发到OpenClaw、MCP或Internal provider。
...
\begin{figure}[tbp]
  \centering
  \begin{tikzpicture}[x=\linewidth,y=7.8cm]
    \foreach \x/\label in {0.08/User,0.31/Main Agent,0.54/Pre/Post Gates,0.77/Subagent \& Tool,0.94/Trace Store} {
      \node[font=\small\bfseries] at (\x,0.97) {\label};
      \draw[afline!60] (\x,0.08) -- (\x,0.92);
    }
    \draw[afarrow] (0.08,0.87) -- (0.31,0.87);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.195,0.90) {1 正常订单查询};
    \draw[afdanger] (0.31,0.79) -- (0.54,0.79);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.425,0.82) {2 RBAC 拦截密钥};
    \draw[afdanger] (0.31,0.71) -- (0.54,0.71);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.425,0.74) {3 参数注入阻断};
    \draw[afdanger] (0.31,0.63) -- (0.54,0.63);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.425,0.66) {4 上下文外泄阻断};
    \draw[afdanger] (0.31,0.55) -- (0.54,0.55);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.425,0.58) {5 重复失败升级};
    \draw[afarrow] (0.54,0.47) -- (0.77,0.47);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.655,0.50) {6 长参数清洗};
    \draw[afarrow] (0.54,0.39) -- (0.31,0.39);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.425,0.42) {7 敏感工具确认};
    \draw[afarrow] (0.31,0.31) -- (0.77,0.31);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.54,0.34) {8 创建子智能体};
    \draw[afarrow] (0.31,0.23) -- (0.77,0.23);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.54,0.26) {9 委派支付分析};
    \draw[afarrow] (0.54,0.15) -- (0.94,0.15);
    \node[font=\scriptsize, fill=white, inner sep=1pt] at (0.74,0.18) {10 Trace 审计落盘};
  \end{tikzpicture}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[draw=afline, fill=afblue2] coordinates {
        ...
        (100,Tool Abuse)
        (100,Data Exfiltration)
        (100,Resource Exhaustion)
        (71.4,Confused Deputy)
        (70.0,Social Engineering)
        (66.7,Advanced Multi-Turn)
        (50.0,RAG Poisoning)
        (33.3,Payload Splitting)
        (16.7,Secrets Detection)
        (16.7,Multi-Language)
        (15.4,Obfuscation)
      };
    \end{axis}
...
\small
\begin{longtable}{>{\centering\arraybackslash}p{0.8cm}>{\RaggedRight\arraybackslash}p{2.5cm}>{\RaggedRight\arraybackslash}p{2.7cm}>{\RaggedRight\arraybackslash}p{2.1cm}>{\RaggedRight\arraybackslash}p{5.0cm}}
...
C07 & 管理员退款 & \code{issueRefund} & \makecell[l]{\decision{ALLOW}\\需按scope校准} & 当前默认检查read scope，写工具确认策略需在生产中进一步按write scope强化。 \\
C08 & 长查询参数 & \code{searchKnowledgeBase} & \decision{MODIFY} & 查询字符串超过500字符后被截断。 \\
C09 & 委派支付分析 & \makecell[l]{\code{delegate\_to\_}\\\code{payments\_analyst}} & \decision{ALLOW} & 运行时将子Agent暴露为委派工具。 \\
...
论文定稿中的核心结构图与统计图均直接由LaTeX内嵌的TikZ/PGFplots生成，编译论文即可得到最终图像。早期草图脚本\code{artical/generate_figures.py}仍可保留作版本对照，但不再作为正文图表的唯一来源。推荐编译命令如下：
...
路径 & 作用 \\
\midrule
\filepath{apps/proxy-service/src/pipeline/graph.py} & Proxy Firewall 9节点LangGraph流水线。 \\
\filepath{apps/proxy-service/src/pipeline/nodes/decision.py} & 风险评分与ALLOW/MODIFY/BLOCK决策。 \\
\filepath{apps/agent/src/agent/graph.py} & Agent Runtime 11节点LangGraph运行图。 \\
...
\filepath{BENCHMARK.md} & 历史完整扫描器基准报告。 \\
\bottomrule
\end{longtable}
\normalsize
